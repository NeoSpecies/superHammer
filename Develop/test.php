...
class UnixSocketReader {
    ...
    public function __construct($socketFile, $poolSize = 5) {
...
// 使用示例
try {
    $socketFile = __DIR__ . '/../runtime/mainSocket.sock';
    $unixSocketCommunication = new UnixSocketReader($socketFile);

    $response = $unixSocketCommunication->sendAndReceive("Message 1");
    echo "Received response: $response\n";

    $response = $unixSocketCommunication->sendAndReceive("Message 2");
    echo "Received response: $response\n";

    $response = $unixSocketCommunication->sendAndReceive("Message 3");
    echo "Received response: $response\n";
} catch (\Exception $e) {
